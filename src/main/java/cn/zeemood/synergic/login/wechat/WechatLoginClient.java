...
public class WechatLoginClient {
	...
	private final static String AUTH_URL_OFFICIAL_URL="https://open.weixin.qq.com/sandbox/connect/oauth2/authorize?appid=%s&redirect_uri=%s&response_type=code&scope=snsapi_login&state=%s#wechat_redirect";
	//替换
	...
	public static String getOfficialAccAuthUrl(String redirect_url,String state)throws Exception{
		...
		try {
			if(redirect_url!=null&&!"".equals(redirect_url)){
				url=url.replaceFirst(RPLC, getOfficialAccAppid()).replaceFirst(RPLC, URLEncoder.encode(getOfficialAccRedirectUrl(), "UTF-8"));
			}else{
				url=url.replaceFirst(RPLC, getOfficialAccAppid()).replaceFirst(RPLC, URLEncoder.encode(redirect_url,"UTF-8"));
			}
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
